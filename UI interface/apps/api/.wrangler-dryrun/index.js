...
function getSql(env) {
  ...
  return cs(env.NEON_DATABASE_URL);
}
...
// src/auth.ts
var DEFAULT_OWNER_EMAIL = "katishay@gmail.com";
var DEFAULT_PASSWORD_ITERATIONS = 1e5;
...
function toHex(bytes) {
  ...
  return Array.from(view).map((b2) => b2.toString(16).padStart(2, "0")).join("");
}
__name(toHex, "toHex");
...
__name(getSessionTtlDays, "getSessionTtlDays");
...
async function ensureOwnerUser(env) {
...
__name(ensureOwnerUser, "ensureOwnerUser");
...
var googleAuthInput = external_exports.object({
  id_token: external_exports.string().min(20).max(4096)
});
...
var NETWORK_REQUIRED_VISIBILITY_FIELDS = ["share_company", "share_role", "share_applied_at"];
function parseAnchorDay(rawAnchor) {
  return rawAnchor && ISO_DATE_REGEX.test(rawAnchor) ? rawAnchor : null;
}
__name(parseAnchorDay, "parseAnchorDay");
var DEFAULT_USER_FIELD_VISIBILITY = {
  share_company: true,
  share_role: true,
...
function toBoolean(value, fallback) {
  ...
  const raw2 = String(value ?? "").trim().toLowerCase();
  if (raw2 === "true" || raw2 === "t" || raw2 === "1" || raw2 === "yes") return true;
  if (raw2 === "false" || raw2 === "f" || raw2 === "0" || raw2 === "no") return false;
  return fallback;
}
__name(toBoolean, "toBoolean");
function normalizeUserFieldVisibility(row) {
  return {
...
__name(normalizeUserFieldVisibility, "normalizeUserFieldVisibility");
function applyRequiredVisibilityFields(visibility) {
  return {
    ...visibility,
    share_company: true,
    share_role: true,
...
__name(applyRequiredVisibilityFields, "applyRequiredVisibilityFields");
async function ensureUserFieldVisibility(env, userId) {
...
__name(ensureUserFieldVisibility, "ensureUserFieldVisibility");
var friendRequestInput = external_exports.object({
  receiver_id: external_exports.coerce.number().int().positive().optional(),
  email: external_exports.string().email().optional()
});
function areSignupsEnabled(env) {
  const raw2 = env.SIGNUPS_ENABLED ?? env.ALLOW_SIGNUPS;
  if (raw2 == null) return true;
  const normalized = String(raw2).trim().toLowerCase();
  return normalized === "true" || normalized === "1" || normalized === "yes";
}
__name(areSignupsEnabled, "areSignupsEnabled");
function getAllowedGoogleClientIds(env) {
  const rawIds = [env.GOOGLE_CLIENT_ID, env.GOOGLE_CLIENT_IDS].filter((value) => Boolean(value)).flatMap((value) => value.split(",")).map((value) => value.trim()).filter(Boolean);
  return new Set(rawIds);
}
__name(getAllowedGoogleClientIds, "getAllowedGoogleClientIds");
function isGoogleEmailVerified(value) {
  if (value === true) return true;
  const normalized = String(value ?? "").trim().toLowerCase();
  return normalized === "true" || normalized === "1";
}
__name(isGoogleEmailVerified, "isGoogleEmailVerified");
...
var targetsUpsertInput = external_exports.object({
  daily_target: external_exports.number().int().min(0).nullable().optional(),
  weekly_target: external_exports.number().int().min(0).nullable().optional(),
  monthly_target: external_exports.number().int().min(0).nullable().optional()
});
...
app.post("/auth/login", async (c) => {
  ...
  const passwordOk = await verifyPassword(
    ...
    user.password_hash,
    user.password_salt,
    Number(user.password_iterations ?? 1e5)
  );
  if (!passwordOk) {
    return c.json({ error: "Invalid email or password." }, 401);
  }
...
app.put("/api/targets", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const p2 = parsed.data;
  if (p2.daily_target === void 0 && p2.weekly_target === void 0 && p2.monthly_target === void 0) {
  ...
  const [row] = await query(
    ...
    [
      userId,
      p2.daily_target === void 0 ? null : p2.daily_target,
      p2.weekly_target === void 0 ? null : p2.weekly_target,
      p2.monthly_target === void 0 ? null : p2.monthly_target
    ]
  );
  return c.json({
    ok: true,
...
var networkFieldVisibilityUpdateInput = external_exports.object({
  share_company: external_exports.boolean().optional(),
  share_role: external_exports.boolean().optional(),
  share_applied_at: external_exports.boolean().optional(),
  share_oa_status: external_exports.boolean().optional(),
  share_oa_deadline: external_exports.boolean().optional(),
  share_referral_used: external_exports.boolean().optional(),
  share_notes: external_exports.boolean().optional()
});
...
app.get("/api/dashboard/summary", async (c) => {
  ...
  const anchorDay = parseAnchorDay(c.req.query("anchorDay"));
  const [referralCount] = await query(env, "SELECT COUNT(*)::text AS count FROM referrals WHERE user_id = $1", [userId]);
  ...
  const [jobsThisWeek] = await query(
    env,
    `
    SELECT COUNT(*)::text AS count
    FROM jobs
  ...
  const [jobsWithReferral] = await query(
    env,
    `
    SELECT COUNT(*)::text AS count
    FROM jobs
  ...
  const [applicationCount] = await query(
    env,
  ...
  const referralDailyTrend = await query(
    env,
    `
  ...
  const rejectedDailyTrend = await query(
    env,
    `
    SELECT d.day::text AS day, COALESCE(r.cnt, 0)::int AS total
    FROM (
  ...
  const pendingDailyTrend = await query(
    env,
    `
  ...
  const referralTrendRaw = await query(
    env,
    `
    SELECT
      CASE
  ...
  const referralTrend = referralOrder.map((status) => {
    const row = referralTrendRaw.find((r) => r.referral_status === status);
    return { referral_status: status, total: row ? row.total : 0 };
  }).concat(
    referralTrendRaw.filter((r) => !referralOrder.includes(r.referral_status) && r.referral_status !== "").map((r) => ({ referral_status: r.referral_status, total: r.total }))
  );
  ...
  const responseStatusTrend = await query(
    env,
    `
  ...
  const oaStatusTrend = await query(
    env,
    `
...
var JOBS_SORT_COLUMNS = ["date_saved", "applied_at", "company", "role", "referral_status", "job_link"];
function isJobsSortColumn(s) {
  return JOBS_SORT_COLUMNS.includes(s);
}
__name(isJobsSortColumn, "isJobsSortColumn");
var CSV_MAX_BYTES = 10 * 1024 * 1024;
var IMPORT_BATCH_SIZE = 200;
var IMPORT_DEFAULT_TIME_SUFFIX = "T00:07:00Z";
var IMPORT_REQUIRED_HEADERS = ["role", "company"];
var IMPORT_DATE_HEADERS = ["date_saved", "applied_at"];
var IMPORT_OPTIONAL_HEADERS = [
  "date_saved",
  "applied_at",
  "location_raw",
  "job_link",
...
var IMPORT_CANONICAL_HEADERS = /* @__PURE__ */ new Set([
...
__name(parseCsvText, "parseCsvText");
function normalizeImportHeader(raw2) {
  return raw2.trim().replace(/^\ufeff/, "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
__name(normalizeImportHeader, "normalizeImportHeader");
function resolveImportHeader(raw2) {
  const normalized = normalizeImportHeader(raw2);
  if (!normalized) return null;
  const aliased = IMPORT_HEADER_ALIASES[normalized];
  if (aliased) return aliased;
  return IMPORT_CANONICAL_HEADERS.has(normalized) ? normalized : null;
}
__name(resolveImportHeader, "resolveImportHeader");
function parseImportDateOnly(raw2) {
...
__name(parseImportDateOnly, "parseImportDateOnly");
function parseImportAppliedAt(raw2, fallbackDate) {
  const trimmed = raw2.trim();
...
__name(parseImportAppliedAt, "parseImportAppliedAt");
...
function csvEscape(value) {
  ...
  const sanitized = /^[\t\r ]*[=+\-@]/.test(raw2) ? `'${raw2}` : raw2;
...
var KEYWORD_MATCHING_ALLOWED = {
  strong: "Strong",
  medium: "Medium",
  week: "Weak",
  weak: "Weak"
};
var OA_STATUS_ALLOWED = {
  yes: "Yes",
  no: "No",
  pending: "Yes",
  complete: "Yes",
...
var APPLICATION_STATUS_ALLOWED = {
  applied: "Applied",
  review: "Review",
  interview: "Interview",
  rejected: "Rejected",
...
__name(normalizeAllowed, "normalizeAllowed");
function normalizeKeywordMatching(value) {
  const raw2 = String(value ?? "").trim().toLowerCase();
  if (!raw2) return null;
  if (raw2 === "strong") return "Strong";
  if (raw2 === "medium") return "Medium";
  if (raw2 === "week" || raw2 === "weak") return "Weak";
  return null;
}
__name(normalizeKeywordMatching, "normalizeKeywordMatching");
function normalizeReferralStatus(value) {
  const raw2 = String(value ?? "").trim().toLowerCase();
  if (!raw2) return null;
  if (raw2 === "requested") return "Requested";
  if (raw2 === "yes") return "Yes";
  return "No";
}
__name(normalizeReferralStatus, "normalizeReferralStatus");
function normalizeOaStatus(value) {
  const raw2 = String(value ?? "").trim().toLowerCase();
  if (!raw2) return null;
  if (raw2 === "yes" || raw2 === "pending" || raw2 === "completed" || raw2 === "complete" || raw2 === "done" || raw2 === "missed" || raw2 === "missing" || raw2 === "overdue") return "Yes";
  return "No";
}
__name(normalizeOaStatus, "normalizeOaStatus");
function asTrimmedString(value) {
  const raw2 = String(value ?? "").trim();
  return raw2 ? raw2 : null;
}
__name(asTrimmedString, "asTrimmedString");
function toIsoDate(value) {
  const raw2 = String(value ?? "").trim();
  if (!raw2) return null;
  if (/^\d{4}-\d{2}-\d{2}$/.test(raw2)) return raw2;
  const parsed = new Date(raw2);
  if (Number.isNaN(parsed.getTime())) return null;
  return parsed.toISOString().slice(0, 10);
}
__name(toIsoDate, "toIsoDate");
async function syncReferralFromJob(env, userId, job, options) {
  ...
  const requestLink = asTrimmedString(job.job_link);
  const requestDate = toIsoDate(job.date_saved) ?? toIsoDate(job.applied_at);
  const keywordMatching = normalizeKeywordMatching(job.keyword_matching) ?? "Medium";
  const comment = asTrimmedString(job.notes);
  ...
  if (!existingId) {
    const [byCompanyRole] = await query(
      env,
      `
      SELECT id
      FROM referrals
...
__name(syncReferralFromJob, "syncReferralFromJob");
function insertJobsImportBatchStatement(userId, rows) {
  return {
    text: `
    INSERT INTO jobs (
      user_id,
...
__name(insertJobsImportBatchStatement, "insertJobsImportBatchStatement");
function syncReferralsFromImportRowsBatchStatement(userId, rows) {
  return {
    text: `
    WITH payload AS (
      SELECT
...
__name(syncReferralsFromImportRowsBatchStatement, "syncReferralsFromImportRowsBatchStatement");
app.get("/api/jobs", async (c) => {
  const userId = c.get("authUser").id;
  const page = Math.max(1, Number(c.req.query("page") ?? 1) || 1);
  const limit = Math.min(Number(c.req.query("limit") ?? 25), 100);
  const company = c.req.query("company");
  const statusFilter = String(c.req.query("status") ?? "");
  ...
  const rows = await query(c.env, `${baseSql}${whereClause}${orderLimitOffset}`, params);
  return c.json({ page, limit, total, data: rows });
});
var jobInput = external_exports.object({
  role: external_exports.string().min(1),
  company: external_exports.string().min(1),
  location_raw: external_exports.string().optional(),
  job_link: external_exports.string().url().optional(),
  job_application_id: external_exports.string().optional(),
  oa_deadline_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  keyword_matching: external_exports.enum(["Strong", "Medium", "Weak", "Week"]).optional(),
  oa_status: external_exports.string().optional(),
  referral_status: external_exports.string().optional(),
  response_status: external_exports.string().optional(),
  application_status: external_exports.string().optional(),
  referred_by_name: external_exports.string().optional(),
  notes: external_exports.string().optional(),
  date_saved: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional()
});
app.post("/api/jobs", async (c) => {
  ...
  const [row] = await query(
    ...
    [
      ...
      p2.role,
      p2.company,
      p2.location_raw ?? null,
      p2.job_link ?? null,
      p2.job_application_id?.trim() ? p2.job_application_id.trim() : null,
      p2.oa_deadline_date ?? null,
      normalizeKeywordMatching(p2.keyword_matching),
      normalizeOaStatus(p2.oa_status),
      normalizeReferralStatus(p2.referral_status),
      p2.response_status ?? null,
      p2.application_status ?? null,
      p2.notes ?? null,
      p2.date_saved ?? null
    ]
  );
  if (row) {
    await syncReferralFromJob(c.env, userId, row, { referredByName: p2.referred_by_name ?? null });
  }
  return c.json(row, 201);
});
var jobsCsvImportInput = external_exports.object({
  csv: external_exports.string().min(1)
});
app.post("/api/jobs/import/csv", async (c) => {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    const csvRow = rows[i];
    const rowNumber = i + 1;
    const getCell = /* @__PURE__ */ __name((field) => {
    ...
    const parsedAppliedAt = parseImportAppliedAt(appliedAtRaw, parsedDateSaved ?? void 0);
    ...
    csvLines.push(
      headers.map((key) => {
        if (key === "keyword_matching") {
          const raw2 = String(row[key] ?? "");
          if (raw2.trim().toLowerCase() === "week") return csvEscape("Weak");
        }
...
var jobUpdateInput = external_exports.object({
  role: external_exports.string().min(1).optional(),
  company: external_exports.string().min(1).optional(),
  location_raw: external_exports.string().optional(),
  job_link: external_exports.string().url().optional().nullable(),
  job_application_id: external_exports.string().optional().nullable(),
  oa_deadline_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
  keyword_matching: external_exports.enum(["Strong", "Medium", "Weak", "Week"]).optional().nullable(),
  oa_status: external_exports.string().optional().nullable(),
  referral_status: external_exports.string().optional().nullable(),
  response_status: external_exports.string().optional().nullable(),
  application_status: external_exports.string().optional().nullable(),
  referred_by_name: external_exports.string().optional().nullable(),
  notes: external_exports.string().optional().nullable(),
  date_saved: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable()
});
...
var oaCompleteInput = external_exports.object({
  oa_completed_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional()
});
...
var oaArchiveUpdateInput = external_exports.object({
  role: external_exports.string().optional().nullable(),
  company: external_exports.string().optional().nullable(),
  location_raw: external_exports.string().optional().nullable(),
  job_link: external_exports.string().optional().nullable(),
  job_application_id: external_exports.string().optional().nullable(),
  oa_deadline_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
  keyword_matching: external_exports.enum(["Strong", "Medium", "Weak", "Week"]).optional().nullable(),
  oa_status: external_exports.string().optional().nullable(),
  ...
  notes: external_exports.string().optional().nullable(),
  date_saved: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
  oa_result: external_exports.enum(["Pending", "Completed", "Missed"]).optional().nullable(),
  oa_result_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
  oa_completed_date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable()
});
app.patch("/api/oa/archive/:id", async (c) => {
  ...
  if (p2.oa_result === "Pending") {
    ...
    let i2 = 1;
    if (p2.role !== void 0) {
      updates2.push(`role = $${i2++}`);
      values2.push(p2.role == null ? null : p2.role.trim());
    }
    if (p2.company !== void 0) {
      updates2.push(`company = $${i2++}`);
      values2.push(p2.company == null ? null : p2.company.trim());
    }
    if (p2.location_raw !== void 0) {
      updates2.push(`location_raw = $${i2++}`);
      values2.push(p2.location_raw == null ? null : p2.location_raw.trim());
    }
    if (p2.job_link !== void 0) {
      updates2.push(`job_link = $${i2++}`);
      values2.push(p2.job_link == null ? null : p2.job_link.trim());
    }
    if (p2.job_application_id !== void 0) {
      updates2.push(`job_application_id = $${i2++}`);
      values2.push(p2.job_application_id == null ? null : p2.job_application_id.trim());
    }
    if (p2.oa_deadline_date !== void 0) {
      updates2.push(`oa_deadline_date = $${i2++}::date`);
      values2.push(p2.oa_deadline_date ?? null);
    }
    if (p2.keyword_matching !== void 0) {
      updates2.push(`keyword_matching = $${i2++}`);
      values2.push(normalizeKeywordMatching(p2.keyword_matching));
    }
    if (p2.referral_status !== void 0) {
      updates2.push(`referral_status = $${i2++}`);
      values2.push(normalizeReferralStatus(p2.referral_status));
    }
    if (p2.response_status !== void 0) {
      updates2.push(`response_status = $${i2++}`);
      values2.push(p2.response_status == null ? null : p2.response_status.trim());
    }
    if (p2.application_status !== void 0) {
      updates2.push(`application_status = $${i2++}`);
      values2.push(p2.application_status == null ? null : p2.application_status.trim());
    }
    if (p2.notes !== void 0) {
      updates2.push(`notes = $${i2++}`);
      values2.push(p2.notes == null ? null : p2.notes.trim());
    }
    if (p2.date_saved !== void 0) {
      updates2.push(`date_saved = $${i2++}::date`);
      values2.push(p2.date_saved ?? null);
    }
    updates2.push(`oa_status = 'Yes'`);
    values2.push(jobId);
    values2.push(userId);
    const [job] = await query(
      c.env,
      `UPDATE jobs SET ${updates2.join(", ")}, updated_at = NOW() WHERE id = $${i2++} AND user_id = $${i2} RETURNING id`,
      values2
    );
  ...
  if (p2.role !== void 0) {
    updates.push(`role = $${i++}`);
    values.push(p2.role == null ? null : p2.role.trim());
  }
  if (p2.company !== void 0) {
    updates.push(`company = $${i++}`);
    values.push(p2.company == null ? null : p2.company.trim());
  }
  if (p2.location_raw !== void 0) {
    updates.push(`location_raw = $${i++}`);
    values.push(p2.location_raw == null ? null : p2.location_raw.trim());
  }
  if (p2.job_link !== void 0) {
    updates.push(`job_link = $${i++}`);
    values.push(p2.job_link == null ? null : p2.job_link.trim());
  }
  if (p2.job_application_id !== void 0) {
    updates.push(`job_application_id = $${i++}`);
    values.push(p2.job_application_id == null ? null : p2.job_application_id.trim());
  }
  if (p2.oa_deadline_date !== void 0) {
    updates.push(`oa_deadline_date = $${i++}::date`);
    values.push(p2.oa_deadline_date ?? null);
  }
  if (p2.keyword_matching !== void 0) {
    updates.push(`keyword_matching = $${i++}`);
    values.push(normalizeKeywordMatching(p2.keyword_matching));
  }
  if (p2.oa_status !== void 0) {
    updates.push(`oa_status = $${i++}`);
    values.push(normalizeOaStatus(p2.oa_status));
  }
  if (p2.referral_status !== void 0) {
    updates.push(`referral_status = $${i++}`);
    values.push(normalizeReferralStatus(p2.referral_status));
  }
  if (p2.response_status !== void 0) {
    updates.push(`response_status = $${i++}`);
    values.push(p2.response_status == null ? null : p2.response_status.trim());
  }
  if (p2.application_status !== void 0) {
    updates.push(`application_status = $${i++}`);
    values.push(p2.application_status == null ? null : p2.application_status.trim());
  }
  if (p2.notes !== void 0) {
    updates.push(`notes = $${i++}`);
    values.push(p2.notes == null ? null : p2.notes.trim());
  }
  if (p2.date_saved !== void 0) {
    updates.push(`date_saved = $${i++}::date`);
    values.push(p2.date_saved ?? null);
  }
  if (p2.oa_result !== void 0) {
    updates.push(`oa_result = $${i++}`);
    values.push(p2.oa_result ?? null);
  }
  if (p2.oa_result_date !== void 0) {
    updates.push(`oa_result_date = $${i++}::date`);
    values.push(p2.oa_result_date ?? null);
  }
  if (p2.oa_completed_date !== void 0) {
    updates.push(`oa_completed_date = $${i++}::date`);
    values.push(p2.oa_completed_date ?? null);
  }
...
var referralInput = external_exports.object({
  company: external_exports.string().min(1),
  request_log: external_exports.string().optional(),
  request_date: external_exports.string().optional(),
  request_link: external_exports.string().url().optional(),
  referral_received: external_exports.string().optional(),
  keyword_matching: external_exports.enum(["Strong", "Medium", "Weak", "Week"]).optional(),
  referred_by_name: external_exports.string().optional(),
...
app.get("/api/referrals", async (c) => {
  ...
  const sql = filter === "open" ? "SELECT * FROM referrals WHERE user_id = $1 AND COALESCE(TRIM(referral_received), '') = 'Requested' ORDER BY request_date DESC NULLS LAST, id DESC LIMIT $2 OFFSET $3" : filter === "applied" ? "SELECT * FROM referrals WHERE user_id = $1 AND COALESCE(TRIM(referral_received), '') = 'Yes' ORDER BY COALESCE(updated_date, request_date) DESC NULLS LAST, id DESC LIMIT $2 OFFSET $3" : "SELECT * FROM referrals WHERE user_id = $1 ORDER BY request_date DESC NULLS LAST, id DESC LIMIT $2 OFFSET $3";
...
app.post("/api/referrals", async (c) => {
  ...
  const [row] = await query(
    ...
    [
      ...
      p2.company,
      p2.request_log ?? null,
      p2.request_date ?? null,
      p2.request_link ?? null,
      normalizeReferralStatus(p2.referral_received),
      normalizeKeywordMatching(p2.keyword_matching),
      p2.referred_by_name ?? null,
      p2.comment ?? null
    ]
  );
  return c.json(row, 201);
...
var referralUpdateInput = external_exports.object({
  company: external_exports.string().min(1).optional(),
  request_log: external_exports.string().optional(),
  request_date: external_exports.string().optional(),
  request_link: external_exports.string().url().optional().nullable(),
  referral_received: external_exports.string().optional().nullable(),
  keyword_matching: external_exports.enum(["Strong", "Medium", "Weak", "Week"]).optional().nullable(),
  referred_by_name: external_exports.string().optional().nullable(),
  comment: external_exports.string().optional()
});
app.patch("/api/referrals/:id", async (c) => {
  ...
  if (p2.company !== void 0) {
    updates.push(`company = $${i++}`);
    values.push(p2.company);
  }
  if (p2.request_log !== void 0) {
    updates.push(`request_log = $${i++}`);
    values.push(p2.request_log);
  }
  if (p2.request_date !== void 0) {
    updates.push(`request_date = $${i++}`);
    values.push(p2.request_date);
  }
  if (p2.request_link !== void 0) {
    updates.push(`request_link = $${i++}`);
    values.push(p2.request_link);
  }
  if (p2.referral_received !== void 0) {
    updates.push(`referral_received = $${i++}`);
    values.push(p2.referral_received == null ? null : normalizeReferralStatus(p2.referral_received));
  }
  if (p2.keyword_matching !== void 0) {
    updates.push(`keyword_matching = $${i++}`);
    values.push(normalizeKeywordMatching(p2.keyword_matching));
  }
  if (p2.referred_by_name !== void 0) {
    updates.push(`referred_by_name = $${i++}`);
    values.push(p2.referred_by_name);
  }
  if (p2.comment !== void 0) {
    updates.push(`comment = $${i++}`);
    values.push(p2.comment);
  }
...
var noteInput = external_exports.object({
  note_date: external_exports.string().optional(),
  comments: external_exports.string().min(1),
  priority: external_exports.enum(["High", "Medium", "Low", "Archive"]).optional(),
  show_on_dashboard: external_exports.boolean().optional()
});
app.post("/api/notes", async (c) => {
  ...
  const [row] = await query(
    ...
    [userId, p2.note_date ?? null, p2.comments, p2.priority ?? "Medium", p2.show_on_dashboard ?? true]
  );
  return c.json(row, 201);
});
var noteEditInput = external_exports.object({
  note_date: external_exports.string().optional(),
  comments: external_exports.string().optional(),
  priority: external_exports.enum(["High", "Medium", "Low", "Archive"]).optional(),
  show_on_dashboard: external_exports.boolean().optional(),
  is_done: external_exports.boolean().optional()
});
app.patch("/api/notes/:id", async (c) => {
  ...
  const values = [];
  let i = 1;
  if (p2.note_date !== void 0) {
    updates.push(`note_date = $${i++}`);
    values.push(p2.note_date);
  }
  if (p2.comments !== void 0) {
    updates.push(`comments = $${i++}`);
    values.push(p2.comments);
  }
  if (p2.priority !== void 0) {
    updates.push(`priority = $${i++}`);
    values.push(p2.priority);
  }
  if (p2.show_on_dashboard !== void 0) {
    updates.push(`show_on_dashboard = $${i++}`);
    values.push(p2.show_on_dashboard);
  }
  if (p2.is_done !== void 0) {
    updates.push(`is_done = $${i++}`);
    values.push(p2.is_done);
  }
  if (updates.length === 0) return c.json({ error: "No fields to update" }, 400);
  values.push(id);
  values.push(userId);
...
var pendingPostInput = external_exports.object({
  company: external_exports.string().min(1),
  position_name: external_exports.string().optional(),
  pending_date: external_exports.string().optional(),
  end_date: external_exports.string().optional(),
  comment: external_exports.string().optional(),
  link: external_exports.union([external_exports.string().url(), external_exports.literal("")]).optional()
});
app.post("/api/pending", async (c) => {
  ...
  const [row] = await query(
    ...
    [
      userId,
      p2.company.trim(),
      p2.position_name?.trim() || null,
      p2.pending_date || null,
      p2.end_date || null,
      p2.comment?.trim() || null,
      p2.link?.trim() || null
    ]
  );
  return c.json(row, 201);
});
var pendingEditInput = external_exports.object({
  company: external_exports.string().min(1).optional(),
  position_name: external_exports.string().optional(),
  pending_date: external_exports.string().optional(),
  end_date: external_exports.string().optional(),
...
app.patch("/api/pending/:id", async (c) => {
  ...
  if (p2.position_name !== void 0) {
    updates.push(`position_name = $${i++}`);
    values.push(p2.position_name);
  }
  if (p2.pending_date !== void 0) {
    updates.push(`pending_date = $${i++}`);
    values.push(p2.pending_date);
  }
  if (p2.end_date !== void 0) {
    updates.push(`end_date = $${i++}`);
    values.push(p2.end_date);
  }
  if (p2.comment !== void 0) {
    updates.push(`comment = $${i++}`);
    values.push(p2.comment);
